...
destination='/mnt/nas/hobe/homelab_backup/moria'
source=("/srv/docker" "/srv/state" "/srv/.git" "/srv/.gitignore" "/home/hobe/backup.sh")
exclude=("/srv/docker/nginx/nginx-config/keys" "/srv/docker/nginx/nginx-config/etc/letsencrypt")
snapshot_name=moria_$(date +%F_%T)
...
/bin/nice -n19 /bin/ionice -c2 -n7 rsync -a -v -h --delete --stats --inplace \
      --link-dest="$destination"/"$latest_snapshot" \
      "${source[@]}" \
      "${excludes[@]}" \
      "$destination"/.unfinished > "$destination"/.unfinished/rsync.log
      #"$destination"/.unfinished > "$destination"/.unfinished/rsync.log 2> >(tee "$destination"/.unfinished/rsync.log)
